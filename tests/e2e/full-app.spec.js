...
async function navigateFromSidebar(page, linkName, routeSuffix, headingName, requiredTexts = []) {
  ...
  await expect(page).toHaveURL(new RegExp(`${escapeRegExp(`${APP_URL}${routeSuffix}`)}(?:\\?.*)?$`));
...
async function createChangeRequestAsFormateur(page, uniqueReason) {
  ...
  await expectNonEmptyMain(page, /Bonjour/i, ['Modules actifs']);
...
test('auth flow rejects invalid credentials, preserves session on reload, and blocks formateur write access to academic config', async ({ page }) => {
  ...
  await test.step('Block direct URL navigation and keep academic configuration read-only for a formateur', async () => {
    ...
    expect(academicConfigResponse.status).toBe(200);
    expect(
      academicConfigResponse.json?.data?.start_date || academicConfigResponse.json?.config?.start_date || '',
    ).not.toBe('');

    const academicConfigWriteResponse = await fetchJsonThroughPage(page, '/api/academic-config', {
      method: 'POST',
      body: {
        start_date: '2025-09-01',
        end_date: '2026-06-30',
        s2_start_date: '2026-02-01',
        stage_start_date: '2026-05-01',
        stage_end_date: '2026-05-31',
        exam_regional_date: '2026-06-15',
      },
    });

    expect(academicConfigWriteResponse.status).toBe(403);
    expect(academicConfigWriteResponse.json?.message).toMatch(/Acces interdit/i);
  });
...
test('mon planning grid stays readonly and avoids exposing invalid workflow actions', async ({ page }) => {
  test.setTimeout(120000);

  const diagnostics = registerDiagnostics(page);

  await test.step('Open Mon Planning and verify the weekly workspace renders as a readonly grid', async () => {
    await loginWithDemoAccount(page, DEMO_USERS.formateur);
    ...
    const main = page.locator('main');
    await expect(main.getByRole('heading', { name: /Mon Planning Hebdomadaire/i })).toBeVisible();
    await expect(main.getByRole('heading', { name: /Alertes Planning/i })).toBeVisible();
  });

  await test.step('Readonly planning never exposes acceptance, refusal, or completion actions', async () => {
    const main = page.locator('main');
    await expect(main.getByRole('button', { name: /Accepter ce creneau/i })).toHaveCount(0);
    await expect(main.getByRole('button', { name: /Refuser ce creneau/i })).toHaveCount(0);
    await expect(main.getByRole('button', { name: /Marquer realise/i })).toHaveCount(0);
  });

  await test.step('Reloading preserves the readonly planning workspace without surfacing forbidden actions', async () => {
    await page.reload({ waitUntil: 'domcontentloaded' });
  ...
  await test.step('Log out after validating the planning grid constraints', async () => {
    await logoutFromSidebar(page);
  });
...
test('formateur journey stays functional across the main pages', async ({ page }) => {
  ...
  await test.step('Open the login page and authenticate with the real demo formateur account', async () => {
    ...
    await page.waitForURL(new RegExp(`${escapeRegExp(`${APP_URL}/formateur`)}$`));
    await expectNonEmptyMain(page, /Bonjour/i, ['Modules actifs', 'Ouvrir mon planning']);
  });
...
test('chef dashboard loads the real operational overview', async ({ page }) => {
  ...
  await test.step('Browse the trainer management page and use the real search control', async () => {
    ...
    await page.getByPlaceholder(/Rechercher par nom, email ou specialite/i).fill('Yassine');
  ...
  await test.step('Browse the modules page and open the intelligent assignment page from the real CTA', async () => {
    ...
    await expect(page.getByRole('heading', { name: /Appariement intelligent/i })).toBeVisible();
    await expect(page.locator('main')).toContainText(
      /Selectionner un module a assigner|Tous les modules sont deja affectes/i,
    );
  });
  ...
  await test.step('Open planning, move to another week, and open the real creation modal', async () => {
    await navigateFromSidebar(
      ...
      ['Cours planifies', 'Planning de la semaine'],
    );
  ...
  await test.step('Review the pending trainer request from the real notifications page', async () => {
    ...
    await expect(page.locator('.chef-notifications-page')).toContainText(uniqueReason);
    const responseInputs = page.getByTestId('chef-notification-response-input');
    const initialActionableCount = await responseInputs.count();
    expect(initialActionableCount).toBeGreaterThan(0);
    await responseInputs.first().fill(reviewNote);
    ...
    await page.getByTestId('chef-notification-approve').first().click();

    const reviewResponse = await reviewResponsePromise;
    expect(reviewResponse.status()).toBe(200);
    await expect(responseInputs).toHaveCount(initialActionableCount - 1);
  });
...
test('directeur dashboard loads the real validation and academic overview', async ({ page }) => {
  ...
  await test.step('Authenticate as the real directeur demo user and open the dashboard', async () => {
    ...
    await expectNonEmptyMain(page, /Tableau de bord directeur|Tableau de Bord - Directeur Pedagogique/i, [
      /Etat du pipeline de validation/i,
      /Signal faible et traces/i,
      /Ouvrir les validations/i,
    ]);
    await expect(page.getByRole('link', { name: /Configurer le calendrier/i })).toBeVisible();
  });
  ...
  await test.step('Open the validation dashboard and inspect a real submission detail when available', async () => {
    ...
    if ((await firstRow.count()) > 0) {
      ...
      if ((await detailModal.count()) > 0) {
        await expect(detailModal).toContainText(/Heures soumises|Soumission courante/i);
        await detailModal.getByRole('button').first().click();
      }
    }
  });

  await test.step('Open the module progression page from the directeur workspace', async () => {
    await page.goto(`${APP_URL}/directeur/progression-modules`, { waitUntil: 'domcontentloaded' });
    await expect(page).toHaveURL(new RegExp(`${escapeRegExp(`${APP_URL}/directeur/progression-modules`)}(?:\\?.*)?$`));
    await expectNonEmptyMain(page, /Progression des Modules/i, ['Progression visuelle']);
  });
  ...
  const expectedDirecteurApiCalls = [
    ...
    '/api/dashboard?action=stats',
    '/api/planning?action=validation-summary',
    '/api/planning?action=validation-queue',
    '/api/planning?action=validation-history',
