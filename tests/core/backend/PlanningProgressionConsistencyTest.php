...
require_once __DIR__ . '/../../../backend/repositories/ReportRepository.php';

use PHPUnit\Framework\TestCase;

final class PlanningProgressionConsistencyTest extends TestCase
{
    private PDO $db;
    ...
    public function testValidatedPlanningOnlyContributesToPlannedTotals(): void
    {
        $fixture = $this->createBaseFixture();

        $this->insertSession($fixture['formateur_id'], $fixture['module_id'], 26, '2026-02-10', 'scheduled');
        $this->insertSession($fixture['formateur_id'], $fixture['module_id'], 27, '2026-02-17', 'scheduled');

        $this->insertSubmission($fixture['formateur_id'], 26, 2026, 'approved', 2.0);
        $this->insertSubmission($fixture['formateur_id'], 27, 2026, 'pending', 2.0);

        $reportRow = $this->findWorkloadRow($fixture['formateur_id']);
        $validatedWeek = $this->dashboard->getTrainerKpis($fixture['formateur_id'], 26, 2026);
        $pendingWeek = $this->dashboard->getTrainerKpis($fixture['formateur_id'], 27, 2026);

        self::assertSame(2.0, floatval($reportRow['planned_hours'] ?? 0));
        self::assertSame(1, intval($reportRow['planned_sessions'] ?? 0));
        self::assertSame(2.0, floatval($validatedWeek['weekly_hours'] ?? 0));
        self::assertSame(0.0, floatval($pendingWeek['weekly_hours'] ?? 0));
    }

    public function testCompletedSessionsUpdateProgressionEvenWhenPlanningIsPending(): void
    {
        $fixture = $this->createBaseFixture();

        $this->insertSession($fixture['formateur_id'], $fixture['module_id'], 28, date('Y-m-d', strtotime('-1 day')), 'done');
        $this->insertSubmission($fixture['formateur_id'], 28, 2026, 'pending', 2.0);

        $moduleRow = $this->findModuleProgressRow($fixture['module_id']);
        $trainerKpis = $this->dashboard->getTrainerKpis($fixture['formateur_id'], 28, 2026);
        $reportRow = $this->findWorkloadRow($fixture['formateur_id']);
        ...
        self::assertSame(0.0, floatval($reportRow['planned_hours'] ?? 0));
    }
    ...
    private function insertFormateur(string $suffix): int
    {
        ...
        $stmt->execute([
            'nom' => 'Consistency Formateur ' . $suffix,
            'email' => 'consistency.' . $suffix . '@example.com',
            'telephone' => '+212611' . substr(md5($suffix), 0, 6),
    ...
    private function insertModule(string $suffix): int
    {
        ...
        $stmt->execute([
            'code' => 'CNS-' . strtoupper($suffix),
            'intitule' => 'Module Consistance ' . $suffix,
            'filiere' => 'Gestion',
            'semestre' => 'S2',
            'volume_horaire' => 10,
            'has_efm' => 0,
    ...
    private function insertSubmission(
        ...
        $stmt = $this->db->prepare(
            'INSERT INTO planning_submissions (
                ...
                :status,
                NOW(),
                CASE WHEN :status_processed = "pending" THEN NULL ELSE NOW() END
             )'
        );
        $stmt->execute([
            'formateur_id' => $formateurId,
